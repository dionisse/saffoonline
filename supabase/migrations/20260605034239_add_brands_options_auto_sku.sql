...
BEGIN
  IF NEW.sku IS NULL OR trim(NEW.sku) = '' THEN
    ...
    LOOP
      ...
      counter := counter + 1;
      IF counter > 100 THEN
        candidate := prefix || '-' || lpad(((extract(epoch FROM clock_timestamp()) * 1000)::bigint % 100000)::text, 5, '0');
        EXIT;
      END IF;
    END LOOP;
...
CREATE TRIGGER trg_auto_sku
  ...
  FOR EACH ROW EXECUTE FUNCTION public.fn_auto_sku();
