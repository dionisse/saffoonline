...
export function AuthPage({ setView }: { setView: (v: View) => void }) {
  const { signIn, signUp, signInWithGoogle, resetPassword, updatePassword, isPasswordRecovery, clearRecoveryMode } = useAuth();
  ...
  async function handleGoogleSignIn() {
    ...
    if (res.error) {
      setError(`Erreur lors de la connexion Google : ${res.error}`);
    ...
    <div className="max-w-lg mx-auto px-4 py-8 lg:py-14 page-enter">
      ...
      <div className="card p-6 sm:p-8 shadow-xl border border-[#E4E7ED] bg-white rounded-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Google Sign-in Button (Electro style) */}
          {(mode === 'signin' || mode === 'signup') && (
            <div>
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white hover:bg-gray-50 text-[#2B2D42] font-bold text-xs uppercase tracking-wider rounded-xl border border-gray-300 shadow-sm hover:shadow transition-all duration-200 active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {googleLoading ? (
                ...
                <span>{mode === 'signup' ? "S'inscrire avec Google" : "Continuer avec Google"}</span>
              </button>

              {/* Divider */}
              <div className="relative my-4 flex items-center justify-center">
                <div className="border-t border-gray-200 w-full" />
                <span className="bg-white px-3 text-[10px] font-extrabold uppercase tracking-widest text-gray-400 absolute">
                  ou par e-mail
                </span>
              </div>
            </div>
